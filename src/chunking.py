...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        for index, raw_part in enumerate(raw_parts):
            suffix = separator.rstrip() if index < len(raw_parts) - 1 else ""
            part = f"{raw_part}{suffix}".strip()
            if not part:
                continue
            if len(part) > self.chunk_size:
                parts.extend(self._split(part, remaining_separators[1:]))
            else:
                parts.append(part)

        joiner = separator if separator.isspace() else " "
        chunks: list[str] = []
        current_chunk = ""
        for part in parts:
            candidate = f"{current_chunk}{joiner}{part}".strip() if current_chunk else part
